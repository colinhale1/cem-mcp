...
const SETS: LibSet[] = [
  {
    ...
    cases: [
      ...
      { query: "show a temporary toast", expect: ["calcite-alert"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
      { query: "modal dialog overlay", expect: ["calcite-dialog", "calcite-modal", "calcite-sheet"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
  ...
  },
  {
    ...
    cases: [
      ...
      { query: "show a temporary notification", expect: ["sl-alert"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
      { query: "rich text editor", expect: ["sl-textarea"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT; sl-textarea is closest" },
  ...
  },
  {
    ...
    cases: [
      { query: "button", expect: ["pf-v5-button"], kind: "anchored", note: "DESCRIPTION-DEPENDENT (v5 prefix)" },
      ...
      { query: "show a notification badge", expect: ["pf-v5-badge"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
    ],
  },
  {
  ...
  },
  {
    ...
    cases: [
      ...
      { query: "show a banner alert", expect: ["nord-banner"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
    ],
  },
  {
    ...
    cases: [
      ...
      { query: "popver", expect: ["ui5-popover"], kind: "anchored", note: "typo; UI5 uses popover for tooltips" },
      { query: "show a temporary message", expect: ["ui5-toast"], kind: "paraphrastic", note: "DESCRIPTION-DEPENDENT" },
    ],
  },
];
...
async function main(): Promise<void> {
  ...
  let totalCases = 0;
  let cust1 = 0, cust3 = 0, fuzz1 = 0, fuzz3 = 0;
  ...
  console.log(`  custom    top-1=${pct(cust1, totalCases)} (${cust1})  top-3=${pct(cust3, totalCases)} (${cust3})`);
  console.log(`  fuzzysort top-1=${pct(fuzz1, totalCases)} (${fuzz1})  top-3=${pct(fuzz3, totalCases)} (${fuzz3})`);
  console.log("\nBy kind:");
